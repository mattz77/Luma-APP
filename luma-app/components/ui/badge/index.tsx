...
type IBadgeProps = React.ComponentPropsWithoutRef<typeof ContextView> &
  VariantProps<typeof badgeStyle> & {
    /** Compat API anterior: mapeado para variant */
    action?: 'primary' | 'secondary' | 'error' | 'success' | 'warning' | 'info';
  };
...
function Badge({
  ...
  let v = variant;
  if (action === 'primary') v = 'default';
  else if (action === 'secondary') v = 'secondary';
  else if (action === 'error') v = 'destructive';
  return (
    <ContextView
      className={badgeStyle({ variant: v, class: className })}
      {...props}
      context={{ variant: v }}
    >
      {children}
