...
type ButtonSize = NonNullable<VariantProps<typeof buttonStyle>['size']>;

/** Compat com API gluestack v3 / Luma: action, variant "solid", tamanhos extras, isDisabled */
...
type IButtonProps = Omit<
  ...
  'context' | 'variant' | 'size' | 'disabled'
> &
  LegacyButtonProps & { className?: string };

function mapLegacyButtonProps({
  variant = 'default',
  size = 'default',
  action,
}: LegacyButtonProps): { variant: ButtonVariant; size: ButtonSize } {
  let v: ButtonVariant =
    variant === 'solid' ? 'default' : (variant as ButtonVariant);
  ...
  if (action) {
    const actionVariant: Record<
      NonNullable<LegacyButtonProps['action']>,
      ButtonVariant
    > = {
      primary: 'default',
      secondary: 'secondary',
...
const Button = React.forwardRef<React.ElementRef<typeof UIButton>, IButtonProps>(
  (
    ...
    const { variant: v, size: sz } = mapLegacyButtonProps({
      variant,
      size,
      action,
    });
    return (
      <UIButton
        ...
        className={buttonStyle({ variant: v, size: sz, class: className })}
        context={{ variant: v, size: sz }}
      />
    );
  }
);
