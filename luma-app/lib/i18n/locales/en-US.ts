import type { Translations } from '../types';

export const enUS: Translations = {
  auth: {
    brand: {
      name: 'Luma',
      tagline: 'Your home, always cared for.',
    },
    login: {
      title: 'Sign In',
      subtitle: 'Enter your email and password to continue',
      email: 'Email',
      password: 'Password',
      forgotPassword: 'Forgot password',
      button: 'Sign In',
      noAccount: "Don't have an account?",
      signUp: 'Sign up',
      continueWith: 'or continue with',
      googleContinue: 'Continue with Google',
    },
    register: {
      title: 'Sign Up',
      subtitle: 'Use proper information to continue',
      fullName: 'Full name',
      email: 'Email address',
      password: 'Password',
      terms: 'By signing up, you are agree to our',
      termsLink: 'Terms & Conditions',
      and: 'and',
      privacyLink: 'Privacy Policy',
      button: 'Create Account',
      continueWith: 'or continue with',
      googleContinue: 'Continue with Google',
      alreadyHaveAccount: 'Already have an Account?',
      signIn: 'Sign in',
      fieldsRequired: 'Please provide name, email and a secure password.',
    },
    forgotPassword: {
      title: 'Reset password',
      subtitle:
        'Enter the email associated with your account. We will send you a link to reset your password.',
      subtitleStep1: 'Enter your account email to receive a 6-digit code.',
      subtitleStep2: 'Enter the code sent to your email to continue.',
      subtitleStep3: 'Create your new password to finish the recovery process.',
      stepLabel: 'Step',
      email: 'Email address',
      code: 'Verification code',
      codePlaceholder: 'Enter the 6 digits',
      newPassword: 'New password',
      confirmPassword: 'Confirm new password',
      button: 'Send link',
      buttonSendCode: 'Send code',
      buttonVerifyCode: 'Verify code',
      buttonResetPassword: 'Reset password',
      resendCode: 'Resend code',
      rememberPassword: 'Remember your password?',
      signIn: 'Sign in',
      emailRequired: 'Enter your registered email to receive the link.',
      emailSent: 'Email sent! Check your inbox to reset your password.',
      emailError: 'Could not send email.',
      codeSent: 'Code sent! Check your email.',
      codeRequestError: 'Could not send the code right now.',
      codeInvalid: 'Enter a valid 6-digit code.',
      codeVerified: 'Code verified successfully.',
      codeVerifyError: 'Invalid or expired code.',
      verificationMissing: 'Please verify the code again before resetting your password.',
      passwordTooShort: 'Password must contain at least 6 characters.',
      passwordMismatch: 'Passwords do not match.',
      passwordUpdated: 'Password updated successfully! Redirecting to sign in...',
      passwordUpdateError: 'Could not reset password.',
    },
  },
  common: {
    loading: 'Loading...',
    error: 'Error',
    success: 'Success',
    cancel: 'Cancel',
    confirm: 'Confirm',
    save: 'Save',
    delete: 'Delete',
    edit: 'Edit',
    back: 'Back',
  },
  errors: {
    invalidCredentials: 'Invalid credentials. Check email and password.',
    emailNotConfirmed: 'Email not confirmed. Check your inbox.',
    invalidEmail: 'Invalid email. Use a valid address.',
    passwordTooShort: 'Password must contain at least 6 characters.',
    userAlreadyRegistered: 'This email is already registered. Sign in or use another address.',
    generic: 'An error occurred. Please try again.',
  },
};

